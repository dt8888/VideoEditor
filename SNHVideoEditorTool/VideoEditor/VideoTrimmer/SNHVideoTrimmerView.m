...
- (void)showNoSmallerTrimMessage {
    ...
    if (has == NO) {
          [MBProgressHUD showOnlyText:@"It can't be cut smaller" view:nil delayTime:0.5f];
    }
...
- (void)notifyDelegateOfDidChange
{
    ...
    if (self.showsTimerView) {
        self.timerView.totalTimeLable.text = [NSString stringWithFormat:@"%.2fs - %.2fs",self.startTime/100,self.endTime/100];
    }
