...
- (void)ajustOutputVideoProperty {
    ...
    AVAssetTrack *assetVideoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] firstObject];
    [self.videoTrack setPreferredTransform:assetVideoTrack.preferredTransform];
    //视频输出尺寸
    if (self.renderRect.size.width == 0 && self.renderRect.size.height == 0) {
        self.videoComposition.renderSize = [self fixRenderSizeWithAsset:asset];
    }else {
        self.videoComposition.renderSize = self.renderRect.size;
    }
...
//    NSLog(@"renderSizeW : %.2f---renderSizeH : %.2f",self.videoComposition.renderSize.width,self.videoComposition.renderSize.height);
    ...
    if (self.videoTransitionType == SNHVideoTransitionTypeDefault) {
       ...
       if (self.renderRect.size.width == 0 && self.renderRect.size.height == 0){
        ...
        {
            CGFloat cropOffX = self.renderRect.origin.x;
            CGFloat cropOffY = self.renderRect.origin.y;
            CGAffineTransform finalTransform = CGAffineTransformMakeScale( 0-cropOffX,  0-cropOffY);
    finalTransform = self.videoTrack.preferredTransform;
//            CGAffineTransform   mixedTransform = CGAffineTransformRotate(finalTransform,M_PI_2);
            [avMutableVideoCompositionLayerInstruction setTransform:finalTransform atTime:kCMTimeZero];
        }
...
//        AVMutableVideoCompositionLayerInstruction * avMutableVideoCompositionLayerInstruction = [AVMutableVideoCompositionLayerInstruction videoCompositionLayerInstructionWithAssetTrack:self.videoTrack];
//        if (self.renderRect.size.width == 0 && self.renderRect.size.height == 0) {
//            CGAffineTransform  translateToCenter = CGAffineTransformMakeTranslation(self.OriginRect.size.height, 0.0);
//            CGAffineTransform    mixedTransform = CGAffineTransformRotate(translateToCenter,M_PI_2);
//               [avMutableVideoCompositionLayerInstruction setTransform:mixedTransform atTime:kCMTimeZero];
//        }else
//        {
//            CGAffineTransform  translateToCenter = CGAffineTransformMakeTranslation(self.renderRect.size.height, 0.0);
//            CGAffineTransform    mixedTransform = CGAffineTransformRotate(translateToCenter,M_PI_2);
//               [avMutableVideoCompositionLayerInstruction setTransform:mixedTransform atTime:kCMTimeZero];
//        }
//
...
////        finalTransform = CGAffineTransformMakeRotation(M_PI_2);
//
//        //把视频图层指令放到视频指令中，再放入视频组合对象中
//        avMutableVideoCompositionInstruction.layerInstructions = [NSArray arrayWithObject:avMutableVideoCompositionLayerInstruction];
//        self.videoComposition.instructions = [NSArray arrayWithObject:avMutableVideoCompositionInstruction];
...
#pragma mark  4.加logo
- (void)addLogo:(NSString*)imageStr{
    ...
    if (self.logoDirection == SNVideoLogoDirectionLeftBottom) {
        overlayLayerFrame = CGRectMake(0, 0, 50, 50);
        
    }else if(self.logoDirection == SNVideoLogoDirectionLeftTop) {
        overlayLayerFrame = CGRectMake(0, videoSize.height -50, 50, 50);
        
    }else if (self.logoDirection == SNVideoLogoDirectionRightBottom) {
        overlayLayerFrame = CGRectMake(videoSize.width - 50, 0, 50, 50);
        
    }else if (self.logoDirection == SNVideoLogoDirectionRightTop) {
        overlayLayerFrame = CGRectMake((videoSize.width - 150)/2, (videoSize.height - 150)/2, 150, 150);
    }
    overlayLayer.frame = overlayLayerFrame;
    UIImage *animationImage = [UIImage imageNamed:imageStr];
...
#pragma mark 5.导出视频
- (void)exportVideo:(NSString*)fileType  addLogo:(NSString*)imageStr AsynchronouslyWithSuccessBlock:(void (^)(NSURL *outputURL))successBlock
                                     failureBlock:(void (^)(NSError *error))failureBlock {
...
- (CGSize)fixRenderSizeWithAsset:(AVAsset *)videoAsset{
 NSInteger degrees = [self degressFromVideoFileWithAsset:videoAsset];
    ...
    if(self.renderRect.size.width!=0&&self.renderRect.size.height!=0){
        videoSize = self.renderRect.size;
    }
    CGSize size;
    if (degrees == 90) {
        // 顺时针旋转90°
        ...
        // 顺时针旋转180°
        size = CGSizeMake(videoSize.width,videoSize.height);
    } else if(degrees == 270){
        // 顺时针旋转270°
        size = CGSizeMake(videoSize.height,videoSize.width);
    }else {
...
//    CGFloat cropOffX =self.renderRect.origin.x;
//    CGFloat cropOffY = self.renderRect.origin.y;
//    translateToCenter = CGAffineTransformMakeTranslation(0 - cropOffX, 0 - cropOffY);
    ...
    if (degrees == 90) {
        // 顺时针旋转270°
        translateToCenter = CGAffineTransformMakeTranslation(videoSize.height, 0.0);
        mixedTransform = CGAffineTransformRotate(translateToCenter,M_PI_2);
      
      
    } else if(degrees == 180){
        // 顺时针旋转180°
     translateToCenter = CGAffineTransformMakeTranslation(videoSize.width, videoSize.height);
        mixedTransform = CGAffineTransformRotate(translateToCenter,M_PI);
    } else if(degrees == 270){
        // 顺时针旋转90°
        translateToCenter = CGAffineTransformMakeTranslation(0.0, videoSize.width);
