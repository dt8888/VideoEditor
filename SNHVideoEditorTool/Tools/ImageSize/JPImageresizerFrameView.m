...
- (CGRect)baseImageresizerFrame {
    if (_isArbitrarily) {
        if(self.getCutRect){
            self.getCutRect(CGRectMake(0, 0, self.originImageFrame.size.width,self.originImageFrame.size.height));
        }
        return self.originImageFrame;
    } else {
        CGFloat w = 0;
        CGFloat h = 0;
        _baseImageH = self.imageView.frame.size.height;
        _baseImageW = self.imageView.frame.size.width;
        ...
        CGFloat x = self.maxResizeX + (self.maxResizeW - self.imageresizerFrame.size.width) * 0.5;
        if(x<0){
            x =0;
        }
        CGFloat y = self.maxResizeY + (self.maxResizeH - self.imageresizerFrame.size.height) * 0.5;
        if(y<0){
            y = 0;
        }
        if(self.getCutRect){
            self.getCutRect(CGRectMake(x, y, w, h));
        }
...
- (void)endedImageresizer {
    ...
    if(self.getCutRect){
        self.getCutRect(CGRectMake(self.imageresizerFrame.origin.x-self.originImageFrame.origin.x, self.imageresizerFrame.origin.y-self.originImageFrame.origin.y, self.imageresizerFrame.size.width, self.imageresizerFrame.size.height));
    }
...
//            if(self.getCutRect){
//                self.getCutRect(CGRectMake(x, y , width, height));
//            }
            break;
        }
