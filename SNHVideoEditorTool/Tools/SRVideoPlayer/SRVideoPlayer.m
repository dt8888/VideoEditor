...
- (void)setupSubViews {
    ...
    //测试
    [_renderer start];
    _timer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(autoSendBarrage) userInfo:nil repeats:YES];
    [self.playerLayerView addSubview:_renderer.view];
    [SRBrightnessView sharedBrightnessView];
}

//-(void)setIsShowDanMu:(BOOL)isShowDanMu
//{
//    if(isShowDanMu){
//        [_renderer start];
//        _timer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(autoSendBarrage) userInfo:nil repeats:YES];
...
-(void)setDanMuText:(NSString *)danMuText
{
    if(danMuText!=nil){
        BarrageDescriptor * descriptor = [[BarrageDescriptor alloc]init];
        descriptor.spriteName = NSStringFromClass([BarrageWalkTextSprite class]);
        descriptor.params[@"text"] = danMuText;
...
-(void)autoSendBarrage{
    BarrageDescriptor * descriptor = [[BarrageDescriptor alloc]init];
    descriptor.spriteName = NSStringFromClass([BarrageWalkTextSprite class]);
    NSArray *arrDanMuArr = @[@"我是第一个哇！",@"好精彩😯！",@"挺好玩的吧！",@"哎！生活啊！"];
  descriptor.params[@"text"] = [arrDanMuArr objectAtIndex:arc4random() % 4];
    descriptor.params[@"textColor"] = kRandomColor;
    descriptor.params[@"speed"] = @(100 * (double)random()/RAND_MAX+50);
    descriptor.params[@"direction"] = @(BarrageWalkDirectionR2L);
    descriptor.params[@"side"] = @(BarrageWalkSideRight);
